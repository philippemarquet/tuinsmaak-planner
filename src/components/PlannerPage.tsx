// src/components/PlannerPage.tsx (VOLLEDIG NIEUW) — met tab "Oogstagenda"
// Zelfde look&feel als de bestaande Planner, met extra kalenderweergave
// voor alle gewassen die "in oogst" zitten tussen start- en einddatum.
...
import { format, addDays as dfAddDays, startOfMonth, endOfMonth, startOfWeek, endOfWeek, isSameDay, isSameMonth, isBefore, isAfter } from "date-fns";
...
import { Edit3, Trash2, ChevronDown, Info, AlertTriangle, X, CalendarIcon, Search, ChevronLeft, ChevronRight, Leaf, Sprout } from "lucide-react";

/* =========================================
   Icon helpers + overlay (consistent met Dashboard/Planner)
========================================= */
...
/** Diamant-tiling overlay met <img> iconen (gebruikt in List/Map) */
function IconTilingOverlay({ iconUrl, segmentsUsed = 1, densityPerSegment = 10, maxIcons = 100, minIcons = 6, opacity = 0.9, }: { iconUrl: string; segmentsUsed?: number; densityPerSegment?: number; maxIcons?: number; minIcons?: number; opacity?: number; }) {
  ...
  return (
    <div ref={ref} className="absolute inset-0 pointer-events-none select-none overflow-hidden z-10">
      {items.map((pt, idx) => (
        <img key={idx} src={iconUrl} alt="" draggable={false} style={{ position: "absolute", left: pt.x - pt.size / 2, top: pt.y - pt.size / 2, width: pt.size, height: pt.size, opacity, objectFit: "contain", filter: "drop-shadow(0 0 0.5px rgba(0,0,0,0.15))", }} />
      ))}
    </div>
  );
}
...
function DroppableSegment({ id, occupied, children }: { id: string; occupied: boolean; children: React.ReactNode }) {
  ...
  return (
    <div ref={setNodeRef} className={`relative border border-dashed rounded min-h-[18px] flex items-center justify-center transition-all duration-150 ${ isOver ? "border-primary bg-primary/10 scale-[1.01]" : occupied ? "border-emerald-300/50 bg-emerald-50/20" : "border-muted-foreground/15 bg-muted/10 hover:border-muted-foreground/25" }`}>
      {children}
    </div>
  );
}
function MapDroppable({ id }: { id: string }) {
  const { setNodeRef, isOver } = useDroppable({ id });
  return (
    <div ref={setNodeRef} className={`w-full h-full transition-colors duration-150 ${isOver ? "bg-primary/20" : "bg-transparent"}`} />
  );
}

/* =============================
   NIEUW: OOGSTAGENDA VIEW
   - Maandkalender
   - Toont per dag alle plantingen die "in oogst" zijn
   - Gebruikt (actual_* indien aanwezig) anders (planned_*)
   - Zelfvoorzienend, geen extra file nodig
============================= */
function HarvestAgendaView({
  beds,
  seeds,
  plantings,
  cropTypes,
  ...
  const matchFilters = (seed?: Seed, bed?: GardenBed) => {
    ...
    if (cropTypeFilters.length > 0) {
      const inSel = seed.crop_type_id ? cropTypeFilters.includes(seed.crop_type_id) : cropTypeFilters.includes("__none__");
      if (!inSel) return false;
    }
    return true;
  };

  function effectiveHarvestStart(p: Planting) { return p.actual_harvest_start ?? p.planned_harvest_start ?? null; }
  function effectiveHarvestEnd(p: Planting) { return p.actual_harvest_end ?? p.planned_harvest_end ?? null; }

  // Bouw dag->entries index
  const days: Date[] = [];
  for (let d = gridStart; !isAfter(d, gridEnd); d = dfAddDays(d, 1)) days.push(d);
  ...
  const byDay = useMemo(() => {
    ...
    for (const p of plantings) {
      ...
      // ignore onbelangrijke inversies
      if (isAfter(s, e)) continue;

      // Snij alleen met grid (performant genoeg voor typical aantallen)
      const from = isBefore(s, gridStart) ? gridStart : s;
      const to = isAfter(e, gridEnd) ? gridEnd : e;
      ...
      const color = p.color?.startsWith("#") || p.color?.startsWith("rgb") ? (p.color as string) : (seed?.default_color?.startsWith("#") ? seed.default_color! : "#22c55e");

      for (let d = new Date(from); !isAfter(d, to); d = addDays(d, 1)) {
        const k = toISO(d);
        const arr = map.get(k);
        if (!arr) continue;
        arr.push({ planting: p, seed, bed, iconUrl, color });
      }
    }
    // sort per dag op seednaam
    for (const [k, arr] of map) {
      arr.sort((a, b) => (a.seed?.name || "").localeCompare(b.seed?.name || "", "nl"));
      map.set(k, arr);
    }
    return map;
  ...
  for (let d = gridStart; !isAfter(d, gridEnd); ) {
    const wk: Date[] = [];
    for (let i = 0; i < 7; i++) { wk.push(d); d = dfAddDays(d, 1); }
    weeks.push(wk);
  }
    ...
    <section className="space-y-4">
      ...
      <div className="grid grid-cols-7 gap-[1px] bg-border rounded-lg overflow-hidden">
        {weeks.flat().map((day, idx) => {
          ...
          const maxInline = 4; // toon max 4 en dan "+N"
          const spill = Math.max(0, entries.length - maxInline);

          return (
            <div key={idx} className={cn("min-h-[110px] bg-card p-2 flex flex-col", outOfMonth && "bg-muted/40")}> 
              {/* day badge */}
              <div className="flex items-center justify-between mb-1">
                <span className={cn("text-xs font-medium px-1.5 py-0.5 rounded", today ? "bg-primary text-primary-foreground" : "text-muted-foreground")}>{format(day, "d", { locale: nl })}</span>
              </div>

              {/* entries */}
              <div className="flex flex-col gap-1 overflow-hidden">
                {entries.slice(0, maxInline).map((e, i) => (
                  <div key={e.planting.id + i} className="group flex items-center gap-1 px-1.5 py-1 rounded border text-[11px] bg-white/60 hover:bg-white/90 transition-colors" style={{ borderColor: e.color + "55" }} title={`${e.seed?.name ?? "—"} • ${e.bed?.name ?? "—"}`}>
                    <div className="relative w-3.5 h-3.5 rounded-sm overflow-hidden flex-shrink-0" style={{ background: e.color }}>
                      {e.iconUrl ? (
                        <img src={e.iconUrl} alt="" className="absolute inset-0 m-auto w-3 h-3 object-contain opacity-95" />
                      ) : (
                        <Leaf className="absolute inset-0 m-auto w-3 h-3 text-white/90" />
                      )}
                    </div>
                    <span className="truncate">{e.seed?.name ?? "—"}</span>
                    {e.bed && (
                      <span className="truncate text-muted-foreground ml-auto">{e.bed.name}</span>
                    )}
                  </div>
                ))}

                {spill > 0 && (
                  <DayOverflowPopover day={day} entries={entries} />
                )}
...
function DayOverflowPopover({ day, entries }: { day: Date; entries: Array<{ planting: Planting; seed?: Seed; bed?: GardenBed; iconUrl: string | null; color: string; }> }) {
  return (
    <Popover>
      <PopoverTrigger asChild>
        <button className="text-[10px] text-primary underline self-start">+{Math.max(0, entries.length - 4)} meer</button>
      </PopoverTrigger>
      <PopoverContent className="w-80 p-2">
        <div className="text-xs font-medium mb-2">{format(day, "EEEE d MMMM yyyy", { locale: nl })}</div>
        <div className="space-y-1 max-h-80 overflow-auto">
          {entries.map((e, i) => (
            <div key={e.planting.id + "-ov-" + i} className="flex items-center gap-2 px-2 py-1 rounded border bg-card">
              <div className="relative w-4 h-4 rounded-sm overflow-hidden flex-shrink-0" style={{ background: e.color }}>
                {e.iconUrl ? (
                  <img src={e.iconUrl} alt="" className="absolute inset-0 m-auto w-3.5 h-3.5 object-contain opacity-95" />
                ) : (
                  <Leaf className="absolute inset-0 m-auto w-3.5 h-3.5 text-white/90" />
                )}
              </div>
              <div className="min-w-0">
                <div className="text-[12px] font-medium truncate">{e.seed?.name ?? "—"}</div>
                <div className="text-[11px] text-muted-foreground truncate">{e.bed?.name ?? "—"}</div>
              </div>
            </div>
          ))}
        </div>
...
export function PlannerPage({ 
  ...
  const filteredSeeds = useMemo(() => {
    ...
    if (cropTypeFilters.length > 0) {
      arr = arr.filter((s) => {
        ...
        const hasCt = (cropTypeFilters.includes("__none__") && !s.crop_type_id) || cropTypeFilters.includes(s.crop_type_id ?? "");
  ...
  const gotoToday = () => { const n = new Date(); const d = new Date(n); d.setDate(n.getDate() - ((n.getDay() || 7) - 1)); setCurrentWeek(d); };

  /* ===== DND ===== */
  function handleDragStart(ev: any) { setActiveDragId(String(ev.active?.id ?? "")); }
  function handleDragEnd(ev: any) {
    const over = ev.over; const active = String(ev.active?.id ?? ""); setActiveDragId(null);
    if (!over || !active.startsWith("seed-")) return; const seedId = active.replace("seed-", "");
  ...
  async function handleConfirmPlanting(opts: {
    ...
    startSegment: number; segmentsUsed: number; method: "direct" | "presow"; dateISO: string; color: string; bedIdOverride?: string;
  }) {
      ...
      <div className="sticky top-0 h-screen overflow-hidden flex flex-col">
        ...
        <div className="px-3 py-3 border-b border-border/30 space-y-3">
          ...
          <div className="relative">
            ...
            <input className="w-full pl-8 pr-3 py-2 text-xs bg-muted/30 border-0 rounded-lg focus:ring-2 focus:ring-primary/20 focus:bg-background transition-all placeholder:text-muted-foreground/50" value={q} onChange={(e) => setQ(e.target.value)} placeholder="Zoek op naam…" />
          </div>
          {/* Toggle Pills */}
          <div className="flex flex-wrap gap-1.5">
            <button onClick={() => setInStockOnly(!inStockOnly)} className={cn("px-2.5 py-1 text-[10px] font-medium rounded-full transition-all", inStockOnly ? "bg-primary text-primary-foreground shadow-sm" : "bg-muted/50 text-muted-foreground hover:bg-muted")}>Voorraad</button>
            <button onClick={() => setGreenhouseOnly(!greenhouseOnly)} className={cn("px-2.5 py-1 text-[10px] font-medium rounded-full transition-all", greenhouseOnly ? "bg-emerald-500 text-white shadow-sm" : "bg-muted/50 text-muted-foreground hover:bg-muted")}>Kas</button>
          </div>
          {/* Segmented Control - In Planner */}
          <div className="flex p-0.5 bg-muted/40 rounded-lg">
            {(["all", "planned", "unplanned"] as InPlanner[]).map((k) => (
              <button key={k} className={cn("flex-1 px-2 py-1.5 text-[10px] font-medium rounded-md transition-all", inPlanner === k ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground")} onClick={() => setInPlanner(k)}>
          ...
          <Popover>
            <PopoverTrigger asChild>
              <button className="w-full px-3 py-2 text-xs text-left rounded-lg flex justify-between items-center bg-muted/30 hover:bg-muted/50 transition-all group">
                <span className={cn("truncate", cropTypeFilters.length === 0 ? "text-muted-foreground" : "text-foreground font-medium")}>
                  {cropTypeFilters.length === 0 ? "Alle gewastypen" : cropTypeFilters.length === 1 ? cropTypes.find((ct) => ct.id === cropTypeFilters[0])?.name || "Overig" : `${cropTypeFilters.length} geselecteerd`}
                </span>
            ...
            <PopoverContent className="w-52 p-2 max-h-56 overflow-y-auto bg-popover/95 backdrop-blur-sm border-border/50 z-50">
              <div className="space-y-0.5">
                {cropTypeFilters.length > 0 && (
                  <button onClick={() => setCropTypeFilters([])} className="w-full text-left text-[11px] text-primary hover:underline px-2 py-1 mb-1">Wis selectie</button>
                )}
                {cropTypes.map((ct) => (
                  <label key={ct.id} className="flex items-center gap-2 px-2 py-1.5 rounded-md cursor-pointer hover:bg-muted/50 transition-colors">
                    <Checkbox checked={cropTypeFilters.includes(ct.id)} onCheckedChange={(checked) => { if (checked) setCropTypeFilters([...cropTypeFilters, ct.id]); else setCropTypeFilters(cropTypeFilters.filter((id) => id !== ct.id)); }} className="h-3.5 w-3.5 rounded" />
                ...
                <label className="flex items-center gap-2 px-2 py-1.5 rounded-md cursor-pointer hover:bg-muted/50 transition-colors">
                  <Checkbox checked={cropTypeFilters.includes("__none__")} onCheckedChange={(checked) => { if (checked) setCropTypeFilters([...cropTypeFilters, "__none__"]); else setCropTypeFilters(cropTypeFilters.filter((id) => id !== "__none__")); }} className="h-3.5 w-3.5 rounded" />
          ...
          <Popover>
            <PopoverTrigger asChild>
              <button className="w-full px-3 py-2 text-xs text-left rounded-lg flex justify-between items-center bg-muted/30 hover:bg-muted/50 transition-all group">
                <span className={cn("truncate", selectedMonths.length === 0 ? "text-muted-foreground" : "text-foreground font-medium")}>
                  {selectedMonths.length === 0 ? "Alle maanden" : selectedMonths.length === 1 ? new Date(2000, selectedMonths[0] - 1, 1).toLocaleString("nl-NL", { month: "long" }) : `${selectedMonths.length} maanden`}
                </span>
                <ChevronDown className="h-3.5 w-3.5 text-muted-foreground group-hover:text-foreground transition-colors" />
              </button>
            </PopoverTrigger>
            <PopoverContent className="w-52 p-2 max-h-56 overflow-y-auto bg-popover/95 backdrop-blur-sm border-border/50 z-50">
              <div className="space-y-0.5">
                {selectedMonths.length > 0 && (
                  <button onClick={() => setSelectedMonths([])} className="w-full text-left text-[11px] text-primary hover:underline px-2 py-1 mb-1">Wis selectie</button>
                )}
                {Array.from({ length: 12 }, (_, i) => i + 1).map((m) => (
                  <label key={m} className="flex items-center gap-2 px-2 py-1.5 rounded-md cursor-pointer hover:bg-muted/50 transition-colors">
                    <Checkbox checked={selectedMonths.includes(m)} onCheckedChange={(checked) => { if (checked) setSelectedMonths([...selectedMonths, m].sort((a, b) => a - b)); else setSelectedMonths(selectedMonths.filter((month) => month !== m)); }} className="h-3.5 w-3.5 rounded" />
                    <span className="text-xs capitalize">{new Date(2000, m - 1, 1).toLocaleString("nl-NL", { month: "long" })}</span>
                  </label>
                ))}
              </div>
            </PopoverContent>
        ...
        <div className="flex-1 overflow-y-auto px-2 py-2 space-y-1">
          {filteredSeeds.length === 0 ? (
            ...
            filteredSeeds.map((seed) => (
              <DraggableSeed key={seed.id} seed={seed} isDragging={activeDragId === `seed-${seed.id}`} onInfoClick={() => setSeedDetailsModal(seed)} />
            ))
          )}
        </div>
        {/* Footer count */}
  ...
  const listViewContent = (
    <div className="p-4 pb-8">
      <div className="space-y-6">
        {([["Buiten", outdoorBeds], ["Kas", greenhouseBeds]] as const).map(([label, bedList]) =>
          bedList.length > 0 && (
                  ...
                  return (
                    <div key={bed.id} className="p-2 border rounded-lg bg-card shadow-sm">
                      ...
                      <div className="grid gap-0.5" style={{ gridTemplateRows: `repeat(${bed.segments}, minmax(20px, auto))` }}>
                        {segs.map((i) => {
                          const here = activePlantings.filter((p) => { const s = p.start_segment ?? 0, u = p.segments_used ?? 1; return i >= s && i < s + u; });
                          const ghosts = futurePlantings.filter((p) => { const s = p.start_segment ?? 0, u = p.segments_used ?? 1; return i >= s && i < s + u; });
                            ...
                            <DroppableSegment key={i} id={`bed__${bed.id}__segment__${i}`} occupied={here.length > 0}>
                              <div className="flex flex-col gap-0.5 w-full px-0.5">
                                {here.map((p) => {
                                  ...
                                  return (
                                    <div key={`${p.id}-${i}`} className="relative rounded px-1.5 py-0.5 text-white text-[10px] flex items-center justify-between overflow-hidden" style={{ background: color }} title={`${seed?.name ?? "—"} • ${fmtDMY(p.planned_date)} → ${fmtDMY(p.planned_harvest_end)}`}>
                                      {iconUrl && (<IconTilingOverlay iconUrl={iconUrl} segmentsUsed={1} densityPerSegment={10} opacity={0.85} />)}
                                ...
                                {ghosts.length > 0 && (
                                  <div className="text-white text-[9px] rounded px-1.5 py-0.5" style={{ background: "rgba(34,197,94,.35)", border: "1px dashed rgba(0,0,0,.35)" }}>
                      ...
                      {bedHasConflict(bed.id) && (
                        <div className="mt-1.5 text-[10px] text-red-700">⚠️ Conflicten — <button className="underline" onClick={() => { setView("conflicts"); localStorage.setItem("plannerOpenTab", "conflicts"); }}>Bekijk</button></div>
                      )}
                    </div>
                  );
                })}
  ...
  /* ===== MAP view — visueel gelijk aan eerdere versie ===== */
  function PlannerMap() {
    const viewportRef = useRef<HTMLDivElement | null>(null);
    const CANVAS_W = 3000; const CANVAS_H = 1200;
    const [zoom, setZoom] = useState(() => { const saved = localStorage.getItem("plannerMapZoomV2"); return saved ? parseFloat(saved) : 0.8; });
      ...
      <section className="space-y-3">
        ...
        <div ref={viewportRef} className="relative w-full h-[70vh] rounded-xl border-2 border-amber-800/30 overflow-auto shadow-xl" style={{ background: "linear-gradient(135deg, #2d5016 0%, #3a6b1e 25%, #2d5016 50%, #3a6b1e 75%, #2d5016 100%)" }}>
          <div className="relative" style={{ width: CANVAS_W * zoom, height: CANVAS_H * zoom }}>
            <div className="absolute left-0 top-0" style={{ width: CANVAS_W, height: CANVAS_H, transform: `scale(${zoom})`, transformOrigin: "0 0", borderRadius: 12, backgroundImage: `
              ...
              {beds.map((bed) => {
                ...
                return (
                  <div key={bed.id} className="absolute select-none" style={{ left: x, top: y, width: w, height: h }}>
                    ...
                    <div className="absolute inset-0 rounded-lg" style={{ background: bed.is_greenhouse ? "linear-gradient(135deg, #e8e8e8 0%, #c0c0c0 50%, #e8e8e8 100%)" : `linear-gradient(180deg, #8B6914 0%, #7a5a12 15%, #6d4f0f 30%, #5c4210 50%, #6d4f0f 70%, #7a5a12 85%, #8B6914 100%)`, boxShadow: bed.is_greenhouse ? "0 4px 8px rgba(0,0,0,0.25), inset 1px 1px 0 rgba(255,255,255,0.4)" : "inset 2px 2px 4px rgba(255,255,255,0.15), inset -2px -2px 4px rgba(0,0,0,0.2), 0 4px 8px rgba(0,0,0,0.3)", padding: WOOD_BORDER }}>
                      {!bed.is_greenhouse && (
                        <div className="absolute inset-0 rounded-lg pointer-events-none opacity-30" style={{ backgroundImage: `repeating-linear-gradient(90deg, transparent 0px, transparent 20px, rgba(0,0,0,0.1) 20px, rgba(0,0,0,0.1) 21px), repeating-linear-gradient(0deg, transparent 0px, transparent 3px, rgba(255,255,255,0.05) 3px, rgba(255,255,255,0.05) 4px)` }} />
                      )}
                      <div className="relative w-full h-full rounded-md overflow-hidden" style={{ background: `radial-gradient(ellipse at 30% 40%, rgba(101,67,33,1) 0%, transparent 50%), radial-gradient(ellipse at 70% 60%, rgba(89,60,31,1) 0%, transparent 50%), radial-gradient(ellipse at 50% 30%, rgba(110,75,38,1) 0%, transparent 40%), linear-gradient(180deg, #5c4033 0%, #4a3328 50%, #3e2723 100%)`, boxShadow: "inset 0 2px 8px rgba(0,0,0,0.4)" }}>
                        {bed.is_greenhouse && (
                          <div className="absolute inset-0 pointer-events-none" style={{ background: "linear-gradient(135deg, rgba(255,255,255,0.35) 0%, transparent 30%, transparent 70%, rgba(255,255,255,0.15) 100%)" }} />
                        )}
                        {segCount > 1 && (
                          <div className="absolute inset-0 pointer-events-none" style={{ backgroundImage: vertical ? `repeating-linear-gradient(90deg, transparent 0px, transparent calc(${100 / segCount}% - 1px), rgba(255,255,255,0.08) calc(${100 / segCount}% - 1px), rgba(255,255,255,0.08) calc(${100 / segCount}%))` : `repeating-linear-gradient(0deg, transparent 0px, transparent calc(${100 / segCount}% - 1px), rgba(255,255,255,0.08) calc(${100 / segCount}% - 1px), rgba(255,255,255,0.08) calc(${100 / segCount}%))` }} />
                        )}
                        <div className="absolute inset-0 flex items-start justify-between p-1">
                          <span className="text-[10px] font-semibold px-2 py-0.5 rounded-md" style={{ background: "rgba(255,255,255,0.85)", color: bed.is_greenhouse ? "#2d5016" : "#3e2723", boxShadow: "0 1px 2px rgba(0,0,0,0.15)", maxWidth: "70%", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={bed.name}>{bed.name}</span>
                          <div className="flex items-center gap-1">
                            {bedHasConflict(bed.id) && (
                              <button className="text-[11px] px-1.5 py-0.5 rounded bg-red-600/90 text-white" onClick={(e) => { e.stopPropagation(); setView("conflicts"); localStorage.setItem("plannerOpenTab", "conflicts"); }} title="Conflicten bekijken">⚠️</button>
                            )}
                        ...
                        <div className="absolute inset-0">
                          {active.map((p) => {
                            ...
                            const inset = 1; const segW = vertical ? innerW / segCount : innerW; const segH = vertical ? innerH : innerH / segCount;
                            const rect = vertical ? { top: inset, height: Math.max(1, innerH - inset * 2), left: inset + start * segW, width: Math.max(1, used * segW - inset * 2) } : { left: inset, width: Math.max(1, innerW - inset * 2), top: inset + start * segH, height: Math.max(1, used * segH - inset * 2) };
                            const color = p.color?.startsWith("#") ? p.color : "#22c55e";
                            const hasConflict = (conflictsMap.get(p.id)?.length ?? 0) > 0;
                            const iconUrl = getEffectiveIconUrl(seed, cropTypesById);
                            return (
                              <div key={p.id} className={`absolute rounded text-white text-[10px] px-1 flex items-center ${hasConflict ? "ring-2 ring-red-500 ring-offset-1" : ""} overflow-hidden`} style={{ ...rect, backgroundColor: color }} title={`${seed?.name ?? "—"} • ${fmtDMY(p.planned_date)} → ${fmtDMY(p.planned_harvest_end)}`}>
                                {iconUrl && (<IconTilingOverlay iconUrl={iconUrl} segmentsUsed={used} densityPerSegment={10} opacity={0.88} />)}
    ...
    <div className="h-[calc(100vh-6rem)] flex flex-col overflow-hidden -mx-6 -mb-6">
      ...
      <header className="flex-shrink-0 bg-background border-b z-30 px-6">
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center p-0.5 bg-muted/40 rounded-lg">
              ...
              <span className="px-4 py-2 font-semibold text-sm min-w-[160px] text-center">
                ...
                <span className="text-muted-foreground font-normal">({format(currentWeek, "d MMM", { locale: nl })} - {format(addDays(currentWeek, 6), "d MMM", { locale: nl })})</span>
              </span>
              <button className="px-3 py-2 text-sm font-medium rounded-md hover:bg-background transition-colors" onClick={() => setCurrentWeek(addDays(currentWeek, 7))}>→</button>
            </div>
            <button className="px-3 py-2 text-sm font-medium rounded-lg bg-muted/50 hover:bg-muted text-muted-foreground hover:text-foreground transition-all" onClick={() => { const n = new Date(); const d = new Date(n); d.setDate(n.getDate() - ((n.getDay() || 7) - 1)); setCurrentWeek(d); }}>
              Vandaag
            </button>
          </div>
        </div>
        ...
        <div className="pb-3 flex items-center gap-2">
          {(["list", "map", "timeline", "harvest", "conflicts"] as const).map((k) => {
            const active = view === k; const danger = k === "conflicts" && conflictCount > 0;
            return (
              <button key={k} onClick={() => setView(k)} className={`px-4 py-2 text-sm font-medium rounded-lg transition-all ${ active ? (danger ? "bg-red-600 text-white shadow-sm" : "bg-primary text-primary-foreground shadow-sm") : danger ? "bg-red-50 text-red-700 hover:bg-red-100" : "text-muted-foreground hover:text-foreground hover:bg-muted" }`}>
                {k === "list" ? "Lijstweergave" : k === "map" ? "Plattegrond" : k === "timeline" ? "Timeline" : k === "harvest" ? "Oogstagenda" : "Conflicten"}
                {k === "conflicts" && conflictCount > 0 && (<span className="ml-2 px-1.5 py-0.5 text-xs rounded-full bg-white/20">{conflictCount}</span>)}
              </button>
            );
          })}
          {/* Toekomstige plantingen toggle (voor List/Map) */}
          {(view === "list" || view === "map") && (
            <button onClick={() => setShowGhosts(!showGhosts)} className={cn("ml-auto px-3 py-2 text-sm font-medium rounded-lg transition-all", showGhosts ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground hover:bg-muted/50")}>Toekomstige plantingen</button>
          )}
        </div>
      </header>
      ...
      <DndContext onDragStart={(e)=>setActiveDragId(String(e.active?.id ?? ""))} onDragEnd={handleDragEnd}>
        <div className="flex flex-1 min-h-0">
          ...
          <div className="flex-1 overflow-auto">
            ...
            {view === "harvest" && (
              <div className="p-6">
                <HarvestAgendaView beds={beds} seeds={seeds} plantings={plantings} cropTypes={cropTypes} greenhouseOnly={greenhouseOnly} cropTypeFilters={cropTypeFilters} />
              </div>
            )}
          ...
          <div className="bg-card/95 backdrop-blur-md rounded-2xl shadow-2xl w-full max-w-lg border border-border/50 overflow-hidden animate-in fade-in-0 zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-5 py-4 border-b border-border/30 bg-gradient-to-r from-primary/5 to-transparent">
              <div className="flex items-center gap-3">
                <div className="w-4 h-4 rounded-full ring-2 ring-white shadow-md" style={{ background: popup.seed.default_color?.startsWith("#") ? popup.seed.default_color : "#22c55e" }} />
                <div>
      ...
      {/* Seed Details Modal */}
      {seedDetailsModal && (
        <SeedModal gardenId={garden.id} seed={seedDetailsModal} onClose={() => setSeedDetailsModal(null)} onSaved={async () => { await onDataChange(); setSeedDetailsModal(null); }} />
      )}
    </div>
  );
}
...
function PlantingForm({
  ...
  const [method, setMethod] = useState<"direct" | "presow">(existing?.method ?? (seed.sowing_type === "both" ? "direct" : (seed.sowing_type as "direct" | "presow") ?? "direct"));
  ...
  const startSegmentOptions = useMemo(() => findAllStartSegments(allPlantings, selectedBed, segmentsUsed, plantDate, he, existing?.id), [allPlantings, selectedBed, segmentsUsed, plantDate, he, existing?.id]);

  useEffect(() => { if (!startSegmentOptions.includes(startSegment)) { setStartSegment(startSegmentOptions.length > 0 ? startSegmentOptions[0] : 0); } }, [startSegmentOptions]);

  const monthWarning = useMemo(() => {
    if (!date) return null; const dt = new Date(date); if (Number.isNaN(dt.getTime())) return null;
    const month = dt.getMonth() + 1; // 1-12
    const monthNames = ["","januari","februari","maart","april","mei","juni","juli","augustus","september","oktober","november","december"] as const;
    ...
    if (allowedMonths.length === 0) return null; if (allowedMonths.includes(month)) return null;
    const allowedNames = allowedMonths.map((m) => monthNames[m]).join(", ");
    return { title: "Maand niet geschikt", description: `"${seed.name}" mag niet in ${monthNames[month]} in ${isGreenhouse ? "de kas" : "de volle grond"} (${selectedBed.name}) worden geplant. Toegestane maanden: ${allowedNames}.`, };
          ...
          <SelectContent className="bg-popover/95 backdrop-blur-md border-border/50">
            ...
            {validBeds.map((b) => (
              <SelectItem key={b.id} value={b.id}>
                <span className="flex items-center gap-2">{b.name}{b.is_greenhouse && (<span className="text-[10px] px-1.5 py-0.5 rounded bg-emerald-500/20 text-emerald-600">kas</span>)}</span>
              </SelectItem>
            ))}
          </SelectContent>
        </Select>
      ...
      {seed.sowing_type === "both" ? (
        <div className="space-y-1.5">
          ...
          <div className="flex p-1 bg-muted/30 rounded-lg">
            <button type="button" onClick={() => setMethod("direct")} className={cn("flex-1 px-3 py-2 text-xs font-medium rounded-md transition-all", method === "direct" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground")}>Direct zaaien</button>
            <button type="button" onClick={() => setMethod("presow")} className={cn("flex-1 px-3 py-2 text-xs font-medium rounded-md transition-all", method === "presow" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground")}>Voorzaaien</button>
          </div>
        </div>
      ) : (
        <div className="space-y-1.5">
        ...
        <div className="space-y-1.5">
          ...
          <Popover>
            ...
            <PopoverContent className="w-auto p-0 bg-popover/95 backdrop-blur-md border-border/50" align="start">
              <Calendar mode="single" selected={date ? new Date(date) : undefined} onSelect={(d) => { if (d) setDate(toISO(d)); }} initialFocus className="pointer-events-auto" />
            </PopoverContent>
          </Popover>
