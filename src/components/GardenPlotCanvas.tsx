...
function safeLoadView(prefix: string) {
  const obj = safeLoadJSON(localStorage.getItem(STORAGE_VIEW(prefix)));
  const s = Number(obj?.scale);
  const p = obj?.pan;
  const scale = Number.isFinite(s) && s >= 0.2 && s <= 5 ? s : 1.2;
  const pan =
...
function safeLoadGrid(prefix: string) {
  const obj = safeLoadJSON(localStorage.getItem(STORAGE_GRID(prefix)));
  return {
...
export function GardenPlotCanvas({
  ...
  const onPointerDown = (e: React.PointerEvent) => {
    ...
    if (tool === "ruler") {
      dragState.current = { mode: "ruler", startX: sx, startY: sy, startWorld: w, prevPan: pan };
    ...
    if (single) {
      ...
      if (handlePt) {
        ...
        const dist = Math.hypot(x - (sx + rect.left), y - (sy + rect.top));
        if (dist < HANDLE_SIZE * 1.6) {
          dragState.current = { mode: "rotate", startX: sx, startY: sy, startWorld: w, prevPan: pan };
          return;
        }
      }
    }
    ...
    if (o) {
      ...
      dragState.current = { mode: "drag-objects", startX: sx, startY: sy, startWorld: w, prevPan: pan, altClone: e.altKey };
      (e.target as HTMLElement).setPointerCapture(e.pointerId);
      return;
    }
    if (b) {
      toggleSelect({ kind: "bed", id: b.id }, additive);
      dragState.current = { mode: "drag-beds", startX: sx, startY: sy, startWorld: w, prevPan: pan, altClone: e.altKey };
      (e.target as HTMLElement).setPointerCapture(e.pointerId);
      return;
    }

    if (!additive) clearSelection();
    dragState.current = { mode: "pan", startX: sx, startY: sy, startWorld: w, prevPan: pan };
    (e.target as HTMLElement).setPointerCapture(e.pointerId);
  };
  ...
  const onPointerMove = (e: React.PointerEvent) => {
    ...
    if (ds.mode === "drag-objects") {
      setObjects((prev) =>
        prev.map((o) => (isSelected("obj", o.id) ? { ...o, x: doSnap(o.x + dxw), y: doSnap(o.y + dyw) } : o))
      );
      ...
      if (delta && ids?.length) {
        await Promise.all(
          beds
            ...
            .map((b) => onBedMove(b.id, (b.location_x ?? 0) + delta.dx, (b.location_y ?? 0) + delta.dy))
        ).catch(() => {});
      }
    }
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key.toLowerCase() === "r") {
        if (e.shiftKey) setTool((t) => (t === "ruler" ? "select" : "ruler"));
        else applyRotation(ROT_STEP, e.shiftKey ? ROT_FINE : ROT_STEP);
      }
  ...
  const align = (mode: "left" | "right" | "top" | "bottom" | "hcenter" | "vcenter" | "hspace" | "vspace") => {
    ...
    const getCenter = (s: Exclude<Sel, null>) => {
      if (s.kind === "bed") {
        const b = beds.find((x) => x.id === s.id)!;
        const { dx, dy } = tempMove.current && tempMoveIds.current.includes(b.id) ? tempMove.current : { dx: 0, dy: 0 };
      ...
      <div className="absolute right-3 top-3 z-50 w-72 rounded-xl bg-white/90 border shadow-lg">
        <div className="px-3 py-2 border-b">
          ...
          <div className="text-[11px] text-muted-foreground">
            {selection.length > 1 ? `${selection.length} items geselecteerd` : info?.type}
          </div>
        </div>
