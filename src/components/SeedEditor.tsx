...
export default function SeedEditor({ gardenId, seed, onClose, onSaved }: Props) {
  ...
  const [stockStatus, setStockStatus] = useState<Seed['stock_status']>(seed?.stock_status ?? 'adequate');
    ...
    return (
      <div>
        ...
        <div className="grid grid-cols-6 gap-1">
          {MONTHS.map(m => (
            <label key={m.n} className={`text-xs px-2 py-1 rounded border ${value.includes(m.n) ? 'bg-primary text-primary-foreground border-primary' : 'bg-secondary text-secondary-foreground border-border cursor-pointer'}`}>
              <input
                type="checkbox"
                checked={value.includes(m.n)}
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-sm">Voorraadstatus</label>
                <select className="w-full rounded-md border border-input bg-background px-3 py-2" value={stockStatus} onChange={e=>setStockStatus(e.target.value as any)}>
              ...
              <div>
                ...
                <input type="number" className="w-full rounded-md border border-input bg-background px-3 py-2" value={stockQty} onChange={e=>setStockQty(Number(e.target.value))} />
              </div>
            </div>
